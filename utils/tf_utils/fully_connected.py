...
#     # Infer data using pretrained model
#     def infer(self, dataset, model_path):
        
#         num_test_examples = dataset.test.num_examples
#         network = self.initialise()
#         tf.reset_default_graph()
...
#             utils.tf_utils.saver.load_model(saver, sess, model_path)
            
            
#             batch_x, batch_y = dataset.train.next_batch(num_test_examples)
            
#             batch_x = batch_x.reshape((-1, self.input_shape))
#             batch_y = batch_y.reshape((num_test_examples, self.n_classes))
            

#             output = sess.run(network, feed_dict={self.x_placeholder: batch_x})
